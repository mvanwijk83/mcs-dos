...
function aligned(s) {
  const rows=s.split('\n').filter(r=>r.endsWith('blks)'));
  assert(rows.length>=2);
  for(const r of rows) { assert.equal(r.length,39,r);assert.equal(r.lastIndexOf('('),29,r); }
}
(async()=>{
  ...
  aligned(s);assert(s.includes('ABCDEFGHIJKLMNOP SEQ 100,864 (394 blks)'));
  ...
  aligned(s);assert(/^101 File\(s\) +126,464 bytes \(494 blks\)$/m.test(s),s);
  assert(/^ +43,520 bytes free \(170 blks\)$/m.test(s),s);
