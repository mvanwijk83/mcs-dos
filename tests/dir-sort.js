...
fs.writeFileSync('build/test-dir-sort.c',code);execFileSync('tools/cc65/bin/cl65.exe',['-t','sim6502','-O','-o','build/test-dir-sort','build/test-dir-sort.c'],{stdio:'pipe'});execFileSync('tools/cc65/bin/sim65.exe',['build/test-dir-sort'],{stdio:'inherit'});
assert.equal(JSON.parse(fs.readFileSync('src/command-help.json')).DIR.split('\n\n').at(-1),fs.readFileSync('dir switch help.txt','utf8').replace(/\r\n/g,'\n').trimEnd());
assert(s.includes("POKE(address, 176 + n / 10)"));assert(s.includes('outs(" 01:01  ")'));
assert(s.indexOf("bare = flags & 1",s.indexOf('static void dircmd'))>s.indexOf("if (!diroption(args[i]",s.indexOf('static void dircmd')));
console.log('PASS all 12 sort modes, type/size ties, unsigned sizes, first entry, DIRCMD, invalid options, exact help and EDIT zero padding');
